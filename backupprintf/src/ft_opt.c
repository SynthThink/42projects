...
/*   Updated: 2018/12/14 11:02:49 by malluin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_align(t_keys *lst)
{
	...
	{
		if (ft_strstr(lst->options, "-") != NULL)
			lst->res = minus_padding(&str, length);
		else if (ft_strstr(lst->options, "0") != NULL
		 && ft_strstr(lst->options, ".") == NULL)
			lst->res = zero_padding(&str, length);
		else
			lst->res = simple_padding(&str, length);
	}
}
